...
**Último change completado:** `retire-legacy-visual-v1` (2026-08-14)
...
**Change completado:** `retire-legacy-visual-v1` — retirada del contrato visual V1 completada. Visual Plan V2 es el único contrato visual soportado. Slices 1–6 cerrados (commits `f2a8078`, `1d9fe37`, `86170d3`, `f48f98f`, `9eb1f13`, `d377932`, `bafb2d5`). Baseline funcional **`1181 passed, 0 failed`**. Quinto E2E V2 canónico (job `cmo-2026-08-14-153529`) validó **script V2 PASS** (`55 → 52`, `status=PASS`, `structureValid=true`) y **assets V2 completos** (10/10). El criterio full-E2E se registra como **DEFERRED/WAIVED**: el pipeline quedó bloqueado en `audio` por `AUDIO_DURATION_MISSING`, fuera del scope de retirada V1. `AUDIO_DURATION_MISSING` queda **pospuesto** como trabajo independiente.

**Siguiente prioridad:** mejora de infraestructura de agentes/contexto. Después: iniciar la modularización (`src/shorts_creator/`, `pyproject.toml`). No iniciado todavía.
