...
- Branch full suite after presets/status fix: `1198 passed, 51 skipped, 0 failed`. The skips are legacy bootstrap-compression convergence tests, superseded by active post-TTS fitting coverage. Next exact step: `python3 bin/run_job.py --topic "Cómo se forma un arcoíris" --duration 30 --verbose`.
