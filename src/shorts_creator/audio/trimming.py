...
def trim_narration(
    *,
    ...
    target_chapter_break: float = TARGET_CHAPTER_BREAK_SEC,
    dry_run: bool = False,
) -> int:
    """Trim continuous narration silences and update timing metadata."""
    ...
    for s in cb:
        print(f"    CB {s['startSec']:.3f}-{s['endSec']:.3f} ({s['durationSec']:.3f}s → {target_chapter_break:.3f}s)")
